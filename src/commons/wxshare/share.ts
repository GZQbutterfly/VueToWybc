    ...
    wx.ready(function () {
        if (config && config.hideAllItem === true) {
            ...
            wx.onMenuShareTimeline({
                title: config && config.title ? config.title : document.title, // 分享标题
                link: config && config.link ? config.link : window.location.href.split('#')[0], // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                imgUrl: config && config.imgUrl ? config.imgUrl : '', // 分享图标
            ...
            wx.onMenuShareAppMessage({
                ...
                link: config && config.link ? config.link : window.location.href.split('#')[0], // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                imgUrl: config && config.imgUrl ? config.imgUrl : '', // 分享图标
                desc: config && config.desc ? config.desc : '',
                success: function (res) {
                    // 用户确认分享后执行的回调函数
                    if (config && config.successDone) {
                        config.successDone(res);
