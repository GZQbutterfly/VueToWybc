...
import { isWeiXin, setLocalUserInfo, setAuthUser, getAuthUser, toCMS, getLocalUserInfo, timeout } from '../../../commons/common.env';
...
export class LoginBack extends Vue {
    ...
    mounted() {
        ...
        let result: any = getLocalUserInfo();
    ...
    toPath() {
        let _query: any = this.$route.query;
        ...
        if (_query.type === 'cms') {
            ...
            if (_query.realTo) {
                this.$router.push({ path: _query.realTo, query: _query.realToQuery });
            } else {
                this.$router.push(_query.toPath || '/');
            }
        }
    }
}