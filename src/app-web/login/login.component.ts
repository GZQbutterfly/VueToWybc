...
import { cleanLocalUserInfo, getLocalUserInfo, setLocalUserInfo } from '../../commons/common.env';
...
export class LoginComponent extends BaseVue {
    loginCache = false;
    loginScope = {
        phone: '',
        code: ''
    ...
    mounted() {
        ...
        // 页面执行
        this.$nextTick(() => {
            let loginInput: any = sessionStorage._loginInput;
            if (loginInput) {
    ...
    inputPhone() {
        let loginScope: any = this.loginScope;
    ...
    login() {
        ...
        _self._$service.doLogion(loginScope).then((res) => {
            ...
            if (_reuslt.errorCode) {
                ...
                this.$router.push({ path: 'loginBack', query: this._query });
            }
        });
    }
    setUser(user) {
