        ...
        if (flag) {
            if (shopcartCache) {  //未测试
                ...
                this._$service.synchronousShoppingCart(options).then(res => {
                    ...
                    console.log(res);
                    //获取商品列表
                    _this._$service.getShopcarGoodsesList(1).then((res: any) => {//未测试
                        if (res.data.errorCode) {
                            _this.toast(res.data.msg, false);
                            return;
                        }
    ...
    onSettle() {
        ...
        if (!flag) {
            let dialogObj = {
                ...
                mainFn() {
                    toLogin(_this.$router, { toPath: 'shoppcar', realTo: 'shoppcar' });
                }
            };
            dialog({ dialogObj });
            return;
        ...
        this.$router.push({ path: 'order_submitting', query: { cartId: cartId.join(','), orderSrouce: 'car' } });
    }
    //清空
    onClear() {
        let dialog = this.$store.state.$dialog;
