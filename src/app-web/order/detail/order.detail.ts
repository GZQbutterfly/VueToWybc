...
import { toLogin, getZoneData } from '../../../commons/common.env';
...
export class OrderDetail extends BaseVue {
    ...
    mounted() {
        ...
        this.$nextTick(() => {
            ...
            if (!localStorage._user && this.orderId) {
                toLogin(this.$router, { toPath: '/order_detail?orderNo=' + this.orderId })
                return;
            }
            if (!localStorage._user && this.combinOrderNo) {
                toLogin(this.$router, { toPath: '/order_detail?orderNo=' + this.combinOrderNo })
                return;
            }
            //加载数据
            this.page_reload();
   ...
   msg(){
     ...
     */
    getOrderStateName() {
        return ['', '等待买家付款', '等待商家发货', '商家发货中', '商家已发货', '交易完成', '交易关闭'][this.orderInfo.orders.orderState]
    ...
    goShop(shopId){
        this.$router.push({path:"home",query:{shopId:shopId}});
    }
}
