...
import submitService from './order.submitting.service';
import './order.submitting.scss';
 ...
 */
@Component({
    template: require('./order.submitting.html'),
    components: {
        'address-dialog': AddressDialogComponent,
        'address-operation': Address,
        // 'app-choosePay': ChoosePay
    }
})
export class OrderSubmitting extends BaseVue {
    showNowifi = false;
    orderSrouce: string = 'car';//订单来源（商品goods，购物车car）
    goodsType: string = 'entity'; // 商品类型（entity实物， empty虚拟物品（不能加入购物车，只能直接购物））
    ...
    selectAddressFlag: boolean = false;
    ...
    queryDefaultAddress(addrId?) {
        let _orderAddress: any = this.orderAddress;
        this._$service.queryDefaultAddress(addrId).then((res) => {
            ...
            if (!_result.errorCode) {
                if (_result) {
                    ...
                    _orderAddress.addressInfo = getZoneData(_result);
                } else {
                    this.orderAddress = {};
    ...
    doPackageEverything() {
        this._$service.queryPackage(this.packageId).then((res) => {
            ...
            if (totalMoney > minMoney) {
                ...
                console.log('达到最小消费,优惠' + (redirectMoney / 100).toFixed(2));
            }
            //转换成页面上显示的数据
            let _orderTotal: any = this.orderTotal;
            for (let i = 0; i < len; i++) {
                let _item = this.packageData[i];
                //数量为0的不显示
                if (this.packageNumb[i] > 0) {
                    let showRoll = _item.consuType == 3;
                    let moneyRoll: number = _item.gsMoneyRoll; //'0' || 1 => '0' , 0 || 1 => 1
            ...
            if (_result) {
                _result.then((res) => {
                    ...
                    if (_result.errorCode) {
                        ...
                        let _param = {
                            combinOrderNo: _result.combinOrderNo, orderId: _result.orderId
                        };
    ...
    payCallBack(res) {
        ...
        if (res) {
            //console.log('已支付');
            this.$router.push({ path: 'user_order', query: { listValue: '2' } });
            //this.$router.push({ path: 'order_detail', query: { orderId: _self.payResult.orderId } });
        } else {
            //console.log('未支付');
            this.$router.push({ path: 'user_order', query: { listValue: '1' } });
        }
        //this.$router.push({ path: 'user_order', query: { listValue: '6' } });
    }
    /**
