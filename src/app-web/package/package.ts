...
export class Package extends BaseVue {
    //tag
    packageState: number = -1; // 0固定套餐 1套餐失效 2购买为满足条件 3满足最小消费
    //info
    id: any;
    goodsList: any = [];
    goodsNumb: any = [];
    ...
    totalMoney: number = 0;
    payMoney: number = 0;
    redirectMoney: number = 0;
    //所有的特价商品列表
    allRedirectGoods: any = [];
    ...
    reload(cb = null) {
        ...
        this._$service.getPackageData(this.id).then((res) => {
            ...
            if (end <= now) {
                ...
                this._$service.getPackageGoodsList(this.id).then((res) => {
                    this.goodsList = res.data.data;
                    //test
                    // this.goodsList[0].redirect = 2;
                    //套餐类型
                    this.packageType = this.goodsList[0].packageType;
                    if (this.packageType == 1) {
                        this.minMoney = 0;
                    } else {
                        this.minMoney = this.goodsList[0].minMoney;
                    }
                    this.redirectMoney = 0;
                    //info
                    console.log('套餐类型', ['', '固定套餐', '自选套餐'][this.packageType]);
                    console.log('所有套餐商品', this.goodsList);
                    this.allRedirectGoods = [];
                    //检查是否为固定套餐
                    if (this.packageType == 1) {
                        this.packageState = 0;
                    }
                    ...
                    for (let i = 0; i < len; i++) {
                        ...
                        this.goodsNumb.push(item.number);
                        //check
                        if (item.number == null) {
                            this.goodsNumb[i] = 1;
                        }
                        ...
                        if (this.packageType == 2 && item.packageGsType == 2 && item.checkGoods) {
                            this.goodsNumb[i] = 0;
                        }

                        //筛选出所有特价商品  
                        if (item.redirect == 2) {
                            this.allRedirectGoods.push(item);
                        }
                    ...
                    console.log('所有特价商品', this.allRedirectGoods)
                    //初始化计算一次价格
                    this.setMoney();
                    //refresh
                    cb && cb();
    ...
    goBuy() {
        if (this.packageState != 1) {
            ...
            this.$router.push({ path: 'order_submitting', query: { packageId: this.id, packageNumb: numb, orderSrouce: 'package' } });
        }
        if (this.packageState == 1) {
            let _self = this;
            let dialogObj = {
