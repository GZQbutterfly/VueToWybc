...
import { getLocalUserInfo, timeout, interval, match } from '../../commons/common.env';
...
export class Grade extends BaseVue {
    currentGrade: any = {}; // gradeClass  grade  gradeName userName userImg
    gradeData: any = { tqData: {}, tjList: [], showContent: false };
    gradeList = [];
    status = '';
    leaveTime = 0;
    reDate = 0;
        ...
        this._$service.queryUpInfo().then((res) => {
            ...
            if (_result.errorCode) {
                ...
                if (!_result.ifNeedBail) {
                    ...
                    this.$router.push({ path: 'gradeUp', query: { name: 'M' + this.gradeActive + this.gradeData.gradeName } });
                }
            }
        }).catch(() => {
