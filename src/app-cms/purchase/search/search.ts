...
export class Search extends BaseVue {
    ...
    frm_submit() {
        ...
        if (search) {
            ...
            if (this.$route.query.search) {
                this.$router.replace({ path: 'cmsPurchaseSearch', query: query });
            } else {
                this.$router.push({ path: 'cmsPurchaseSearch', query: query });
            }
        }
    ...
    btn_click() {
        if (this.frm_push) {
            // origin
            this.$router.push({ path: (this.$route.query.origin || 'cmsPurchaseType') });
            event.preventDefault();
            window.event.returnValue = false;
            return false;
        }
