...
export class CmsPurchaseOrderDetail extends BaseVue {
    ...
    mounted() {
        ...
        if (!localStorage._user && this.orderId) {
            toLogin(this.$router, { toPath: '/CmsPurchaseOrderDetail?orderNo=' + this.orderId })
            return;
        }
        if (!localStorage._user && this.combinOrderNo) {
            toLogin(this.$router, { toPath: '/CmsPurchaseOrderDetail?combinOrderNo=' + this.combinOrderNo })
            return;
        }
        //注册服务
